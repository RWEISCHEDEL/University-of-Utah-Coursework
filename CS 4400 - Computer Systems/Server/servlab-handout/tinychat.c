...
/*
 * This method helps append what the user types in chat and adds it to the conversation.
 */
static int appendToConv(dictionary_t *d){
  printf("In appendToConv\n");

  // Get the content and topic and users of this conversation.
  ...
  if(content != NULL && topic != NULL && user != NULL){
    ...
    if(strcasecmp(content, "")){
      ...
      // Get conversation topic from conversation dictionary.
...
/*
 * This method displays the information from the user desired topic to the user.
 */
static void serveTopic(int fd, dictionary_t *query){
  ...
  if(user != NULL && topic != NULL){
    ...
    // Generate the web page filled with the desired chat information.
    body = append_strings("<html><body>\r\n",
                        "<br>TinyChat - ",topic,
                        "\r\n<form action=\"reply\" method=\"post\"",
                        " enctype=\"application/x-www-form-urlencoded\"",
...
/*
 * This method handles conversation requests from the user.
 */
static int serveConv(int fd, dictionary_t *query){
...
/*
 * This method handles the import requests made by the user.
 */
static int serveImport(dictionary_t *query){
  int length;
  int server;
  ...
  // Make a dictionary to store the headers.
  dictionary_t *serverHeaders;

  char *buff;

  // Retieve the topic, host, and port from the url.
  char *topic = dictionary_get(query, "topic");
  ...
  if(topic != NULL && host != NULL && port != NULL){
    ...
    // Ensure that the response is valid, if not close the connection.
